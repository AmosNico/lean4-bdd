...
instance : GetElem BDD (Array Bool) Bool (fun B v ↦ B.nvars ≤ v.size) where
  getElem B v h := (B.lift h).evaluate ⟨v, by simp only [lift_nvars]⟩

lemma getElem_def (B : BDD) (v : Array Bool) (h : B.nvars ≤ v.size) :
    B[v] = (B.lift h).evaluate v.toVector := (rfl)

/-- `lift` does not affect `denotation`. -/
@[simp]
lemma lift_denotation {v : Array Bool} {B : BDD} {h1 : B.nvars ≤ n} {h2 : n ≤ v.size} :
    (B.lift h1)[v] = B[v] := by
  simp [getElem_def, lift, Evaluate.evaluate_evaluate]

/-- `denotation` absorbs `Vector.cast`. -/
@[simp]
lemma denotation_cast {I : Array Bool} {B : BDD} {hn : B.nvars ≤ n} (h : n = m) :
...
def SemanticEquiv (B C : BDD) := ∀ v : Vector Bool (max B.nvars C.nvars), B[v] = C[v]
...
private lemma denotation_append {I : Vector Bool n} {B : BDD} {hn : B.nvars ≤ n} {hm : n ≤ m} {J : Vector Bool (m - n)} :
    B[I] = B[Vector.cast (by omega) (I ++ J)]'(.trans hn hm) := by
  rw [denotation_cast]
  ·
    conv =>
      rhs
