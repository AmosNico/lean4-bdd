...
def SemanticEquiv (B C : BDD) := ∀ n (_ : n ≥ max B.nvars C.nvars) (I : Vector Bool n), B[I] = C[I]
...
theorem SemanticEquiv.equivalence : Equivalence SemanticEquiv :=
  { refl B n h I := rfl,
    ...
    trans := by
      ...
      intro I
      trans C[I]
      · exact hBC _ (by omega) I
      · exact hCD _ (by omega) I
  }
...
@[simp]
lemma const_denotation : ∀ I : Vector Bool n, (const b)[I] = b := by
...
@[simp]
lemma and_denotation {B C : BDD} {I : Vector Bool n} {h : max B.nvars C.nvars ≤ n} :
