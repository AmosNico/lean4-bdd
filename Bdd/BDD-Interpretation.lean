...
structure Interpretation (B : BDD) where
  toArray : Array Bool
  valid : B.nvars ≤ toArray.size

@[simp]
def Interpretation.castLE {B C : BDD} (I : Interpretation B) (h : C.nvars ≤ B.nvars) :
    Interpretation C where
  toArray := I.toArray
  valid := .trans h I.valid

/-- The `denotation` of a `BDD` is the Boolean function that it represents. -/
def denotation (B : BDD) (I : Interpretation B) : Bool := (B.lift I.valid).evaluate ⟨I.toArray, rfl⟩

/-- `lift` does not affect `denotation`. -/
@[simp]
lemma lift_denotation {B : BDD} {h1 : B.nvars ≤ n} :
    ∀ I, (B.lift h1).denotation I = B.denotation (I.castLE  h1) := by
...
lemma denotation_independentOf_of_geq_nvars {n : Nat} {i : Fin n} {B : BDD} {h1 : B.nvars ≤ n} {h2 : B.nvars ≤ i} :
    Nary.IndependentOf B.denotation i := by
  rintro b I
...
def SemanticEquiv (B C : BDD) := B.denotation = C.denotation
