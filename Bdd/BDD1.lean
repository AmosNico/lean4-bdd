...
@[simp]
lemma not_dependsOn_of_ge {B : BDD} {i} (h : i ≥ B.nvars) : ¬ B.DependsOn i := by
  grind only [DependsOn]
...
@[simp]
lemma not_denotation {n} {B : BDD} {h : B.nvars ≤ n} :
...
instance instDecidableDependsOn (B : BDD) : DecidablePred B.DependsOn :=
  fun i ↦
