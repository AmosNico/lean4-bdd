...
def denotation? (B : BDD) (I : Array Bool) : Option Bool :=
  if h : B.nvars ≤ I.size then
    (B.lift h).evaluate I.toVector
  else
    none

lemma denotation_def (B : BDD) (I : Array Bool) :
    B.denotation? I = if h : B.nvars ≤ I.size then some ((B.lift h).evaluate I.toVector) else none := (rfl)
...
lemma lift_denotation {B : BDD} {h1 : B.nvars ≤ n} {h2 : n ≤ m} :
  ...
  ext
  simp only [denotation?, lift, evaluate, Lift.olift_olift, Evaluate.evaluate_evaluate,
    Lift.olift_evaluate, Vector.take_eq_extract, Vector.extract_mk, Nat.sub_zero, Vector.cast_mk]
  split
